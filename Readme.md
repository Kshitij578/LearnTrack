...
Open IntelliJ IDEA.
Click File → Open… and select the LearnTrack project folder.
Ensure the Project SDK is set to your installed JDK (e.g., 22).
In the Project view, open:
src/com/airtribe/learntrack/Main.java
Right-click inside the main method → Run 'Main.main()'.
Use the numbered menu in the Run console to manage students, courses, and enrollments.
Compile and Run from Command Line
From the project root (where src folder exists):

Compile all Java files javac -d out src/com/airtribe/learntrack/.java src/com/airtribe/learntrack//*.java

Run the application java -cp out com.airtribe.learntrack.Main
...
StudentService → StudentRepository → Student CourseService → CourseRepository → Course EnrollmentService → EnrollmentRepository → Enrollment
